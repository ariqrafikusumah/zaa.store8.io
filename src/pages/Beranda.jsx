            ...
            {dataView.map((item, index) => (
              <div key={item.uuid}>
                <div className='rounded-lg mt-3 relative'>
                  <div className=' shdaow-xl rounded-xl bg-yellow-500 shadow-xl'>
                    <div className=''>
                      <img className='relative bottom-5 rounded-xl h-24 w-24 mx-auto shadow-xl' src={item.thumbnail} alt={item.category} />
                    </div>
                    <div className='text-center font-bold'>{item.category}</div>
                    <div className='text-center text-sm mb-2 font-semibold'>{item.sub_category}</div>
                    <div className='text-center'>
                      <a href={item.link}>
                        <button className="text-white bg-blue-900 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-2"> Top Up </button>
                      </a>
                    </div>
                  </div>
                </div>
